...
import { Star, MapPin, Globe, ExternalLink, Bookmark, Check, Sparkles, UserPlus, CheckCircle2 } from "lucide-react";
...
interface Props {
  ...
  onAddNoWebsiteLead?: (b: Business) => void;
  isAddedNoWebsiteLead?: boolean;
}

export default function BusinessCard({ business, onSelect, onSave, isSaved, isPitched, selectable, selected, onToggle, onAddNoWebsiteLead, isAddedNoWebsiteLead }: Props) {
  const content = (
  ...
  return (
    <div className="relative group">
      ...
      {onSelect ? (
        ...
        <div
          className={`w-full bg-[#111] border rounded-xl p-4 ${
            selected ? "border-[#3b82f6] bg-[#3b82f6]/5" : "border-[#222]"
          } ${selectable ? "pl-10" : ""}`}
        >
          {content}
          {onAddNoWebsiteLead && (
            <button
              onClick={(e) => { e.stopPropagation(); onAddNoWebsiteLead(business); }}
              disabled={isAddedNoWebsiteLead}
              className={`w-full mt-3 text-xs font-medium py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5 cursor-pointer disabled:cursor-not-allowed ${
                isAddedNoWebsiteLead
                  ? "bg-[#22c55e]/10 text-[#22c55e] border border-[#22c55e]/20"
                  : "bg-[#3b82f6]/10 text-[#3b82f6] border border-[#3b82f6]/20 hover:bg-[#3b82f6]/20 hover:border-[#3b82f6]/40"
              }`}
            >
              {isAddedNoWebsiteLead ? <CheckCircle2 size={13} /> : <UserPlus size={13} />}
              {isAddedNoWebsiteLead ? "Added to Non-Website Leads" : "Add to Non-Website Leads"}
            </button>
          )}
        </div>
      )}
